...
function showHelpMessage(){
	...
	echo
	echo "--type [시험유형]"
	echo "시험유형을 필수적으로 입력하셔야 합니다. 예로 들면 2018학년도 9월 모의평가 (2017 시행)은 201809sumoi이며, 2017학년도 수능 (2016 시행)은 suneungtnsmd_2017입니다. 형식은 바뀔 수 있으니 평가원 사이트를 참고해 주세요."
	echo
	echo "--subject [과목코드]"
...
function downloadFile(){
	...
	while(true); do
		...
		curl -o "${FILE_PATH}" "${DOWNLOAD_URL}" > /dev/null 2>&1
		if [[ "${COUNT}" == 1 ]]; then
			echo "파일 확인 중..."
